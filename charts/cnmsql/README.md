# CNMSQL Operator chart

A Helm chart for CNMSQL Operator. The chart deploys the operator, its CustomResourceDefinitions, RBAC, webhook configuration, and everything else needed to run MySQL clusters on Kubernetes.
...
|---|---|---|
| `manager.image.repository` | `ghcr.io/cnmsql/cnmsql` | Operator image repository. |
...
```bash
kubectl delete crd clusters.mysql.cnmsql.co \
  backups.mysql.cnmsql.co \
  scheduledbackups.mysql.cnmsql.co \
  databases.mysql.cnmsql.co \
  imagecatalogs.mysql.cnmsql.co \
  clusterimagecatalogs.mysql.cnmsql.co
```
...
- Operator documentation: [cnmsql.co](https://cnmsql.co)
- Source code: [cnmsql/cnmsql](https://github.com/cnmsql/cnmsql)
- OCI chart: `oci://ghcr.io/cnmsql/charts/cnmsql`
